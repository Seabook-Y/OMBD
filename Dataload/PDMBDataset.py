...
class PDMBDataSet(Dataset):
    def __init__(self, args, flag='train'):
        ...
        target_all = pickle.load(
            open(osp.join(self.pickle_root, 'PDMB_' + self.subnet + '_anno.pickle'), 'rb'))
        ...
        for session in self.sessions:
            ...
            for start, end in zip(
                    ...
                    range(seed + self.enc_steps, target.shape[0], 1)):
                enc_target = target[start:end]
                class_h_target = enc_target[self.enc_steps - 1]
                if class_h_target.argmax() != 9:
                    self.inputs.append([session, start, end, enc_target])

        self.feature_All = pickle.load(open(osp.join(
            self.pickle_root, 'PDMB_{}.pickle'.format(self.subnet)), 'rb'))
        print(osp.join(self.pickle_root, 'PDMB_{}.pickle'.format(self.subnet)))
